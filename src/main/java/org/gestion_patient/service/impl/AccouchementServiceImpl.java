...
public class AccouchementServiceImpl implements AccouchementService {
    ...
    public AccouchementDto update(int id, AccouchementDto accouchementDto) {
        Accouchement accouchementToUpdate = accouchementRepository.findById(id).orElseThrow(() -> new ResourceNotFoundException("Accouchement not found with given id " + id));
